...
if (camposObligatoriosValidosParaCliente()) {
  ...
  if (clienteDao.exists(tfCuit.getText().trim())) {
    DialogPopUp.crearDialogo(Alert.AlertType.ERROR, "Error", "Cliente existente", "Ya existe un cliente en la base" + " de datos con el CUIT que usted ingresó");

  } else {

    clienteDao.create(clienteEntity);
    DialogPopUp.crearDialogo(Alert.AlertType.INFORMATION, "Confirmación", "Nuevo cliente creado", "Se ha " + "insertado el nuevo cliente en la base de datos");
  }
